...
<sub>17: これらは他の日本用フィルタでも基本的にブロックされておらず、せいぜい非表示です。今のところ実際には、スポンサー記事でない、一般的なブログにおける商品かサービスについての具体的なレビューに付随するAmazonや楽天へのリンクで、量が過剰でなく、他の関係ない商品やサービスへのリンクを含まないものに限定しています。280blockerさんの[おっしゃるように](https://280blocker.net/advanced-settings/)これらを消すと文章が不自然になるケースもありますし、許可しているわけでもないので（不具合かブロッカー検知以外に許可ルールを使うことはありません）消したい方は「要素をブロック」で簡単に消せるものがほとんどです。</sub>
